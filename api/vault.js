// Chapter 5 web-CTF login — Vercel serverless version of the route that
// ctf-server.js serves locally. Wired up via vercel.json rewrites:
//
//   /vault-7a9/        (GET)  -> this function -> login form
//   /vault-7a9/login   (POST) -> this function -> auth check
//
// The hidden path is disclosed by the static /robots.txt.
//
//   username = zara
//   password = 3rdparty   (an entry in /passwords.txt, so it is brute-forceable)
//
...
export default async function handler(req, res) {
    ...
    if (req.method === 'GET') {
        res.status(200).send(loginPage(null));
        return;
    }
    ...
    // Body may arrive parsed (Vercel) or as a raw urlencoded string.
    let body = req.body;
    ...
    if (user === CTF_USER && pass === CTF_PASS) {
